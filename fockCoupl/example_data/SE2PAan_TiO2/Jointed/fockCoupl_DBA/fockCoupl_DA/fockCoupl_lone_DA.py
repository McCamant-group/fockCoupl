...
for i in range(len(V_tot)):
	## Forward ##
	kf[i] = ((V_tot[i]*np.conj(V_tot[i]))/hbar)*np.sqrt(np.pi/(reorg*kBT))*np.exp(-((reorg+(Eb_orb-Ea[i]))**2/(4*reorg*kBT)))

	## Backward ##
	kb[i] = ((V_tot[i]*np.conj(V_tot[i]))/hbar)*np.sqrt(np.pi/(reorg*kBT))*np.exp(-((reorg+(Ea[i]-Eb_orb))**2/(4*reorg*kBT)))
...
plt.vlines(Eb_orb,0,1) #plt.vlines(Eb[b_orb],0,abs(V_bc)*1e3,colors='b')
